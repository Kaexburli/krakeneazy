...
const generatePassword = (length) => {
  ...
  const password = Array.apply(null, Array(length || 10))
    .map(function () {
...
userSchema.statics.findByToken = async function (token, req) {
  ...
  try {
    const decoded = jwt.verify(token, process.env.JWT_STANDARD_SECRET)
...
// create a custom model method to find user by token for authenticationn
userSchema.statics.findById = async function (id, req) {
  const User = this
...
userSchema.statics.findByForgotToken = async function (resetPasswordToken) {
  ...
  try {
    ...
    const newPassword = generatePassword(15).trim()
