...
const transporter = credentials
  ? nodemailer.createTransport(credentials)
  : false
...
export const sendRegisterEmail = async (user) => {
  ...
  try {
    ...
    const replacements = {
      ...
      line_2:
        'Cliquez sur le lien pour valider votre email et finaliser votre inscription.',
      confirm_link:
        process.env.FRONTEND_URI + '/api/email-confirm/' + confirmationToken,
      btn_label: 'Valider mon email',
      footer:
        "Vous avez reçu ce message car votre adresse email à été utilisée pour s'incrire sur krakeneazy, si vous n'avez pas fait d'inscription veuillez ne pas tenir compte de cet email!"
    }
    ...
    return !!(
      Object.prototype.hasOwnProperty.call(info.envelope, 'from') &&
...
export const sendForgotPasswordEmail = async (user) => {
  ...
  try {
    ...
    const replacements = {
      ...
      line_2:
        'Pour pouvoir réinitialiser votre mot de passe veuillez cliquer sur le lien suivant:',
      forgot_link:
      ...
      footer:
        "Vous avez reçu ce message car votre adresse email à été utilisée pour faire une demande de réinitialisation de mot de passe sur krakeneazy, si vous n'avez pas fait cette demande veuillez ne pas tenir compte de cet email!"
    }
...
export const sendNewPasswordEmail = async (data) => {
  ...
  try {
    ...
    const replacements = {
      ...
      line_2:
        'Veuillez vous connecter avec ce mot de passe et le modifier une fois connecté.',
      line_3: 'Voici votre nouveau mot de passe:',
      newPassword,
      footer:
        "Vous avez reçu ce message car votre mot de passe à été réinitialisé sur krakeneazy, si vous n'avez pas fait cette demande veuillez nous contacter et changer votre mot de passe dans les plus bref délais"
    }
